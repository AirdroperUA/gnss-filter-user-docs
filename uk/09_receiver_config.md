...
**Примітка для F10 / M10 (прошивка 1.6.2+):** новіші покоління u-blox
використовують інтерфейс конфігурації `CFG-VALSET` і не завжди надійно
реагують на застарілий `CFG-MSG` вище для `NAV-SAT`. На додаток до
показаного профілю фільтр надсилає блок `CFG-VALSET`, який заново вмикає
`NAV-SAT` на 5 Гц на активному UART через
`CFG-MSGOUT-UBX_NAV_SAT_UART1/2`. Якщо ви раніше використовували F10 або
M10 без надходження NAV-SAT, оновіться до 1.6.2+ — попереднє налаштування
в u-center не потрібне.
