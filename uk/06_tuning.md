...
| `DR1_MAX_MS` | Макс. тривалість DR1 до примусового виходу (мс, 0=вимкнено) | 0 | 0 | 600000 |
| `FENCE_RAD` | Радіус гео-огорожі від першого фіксу (м, 0=вимкнено) | 600000 | 0 | 2000000 |
...
### Макс. тривалість DR1 та гео-огорожа

- **DR1_MAX_MS**: Максимальний час (мс) перебування в DR1 до примусового виходу в DR0. `0` = вимкнено (за замовчуванням — DR1 тримається поки GPS не відновиться). Типові значення: `60000` (1 хв), `120000` (2 хв), `300000` (5 хв).
- **FENCE_RAD**: Радіус гео-огорожі в метрах від позиції першого фіксу (макс. 2 000 000 м = 2000 км). За замовчуванням `600000` (600 км). Якщо GPS показує позицію за цим радіусом — спрацьовує DR1. `0` = вимкнено. Ловить спуфінг-атаки, які повільно зсувають позицію з часом. Центр огорожі встановлюється при першому фіксі після завантаження — не при позиції активації.
