...
Прошивка обчислює зважену оцінку (0–100) з до 8 сигналів виявлення. Бал видно в Mission Planner як `DR_CONF` і записується в кожен лог подій. Вищі значення означають більше ознак спуфінгу. Для passive NMEA-приймачів, таких як UM980 і Mosaic X5, недоступні UBX-сигнали пропускаються.

### Тільки UBX vs універсальне виявлення

Більшість сигналів працюють з u-blox і passive NMEA-приймачами. Наступні сигнали **тільки для u-blox**:
...
Перевірка узгодженості швидкості та позиції працює з обома класами приймачів, але для u-blox вона точніша, бо використовує NED-швидкість з NAV-PVT. Для passive NMEA-приймачів вона розраховується зі швидкості та курсу RMC.
...
- На u-blox у прошивці v1.6.18+ штатний захищений режим зчитує байти FC GPS back-channel, але не пропускає їх у приймач, тому auto-config записи ArduPilot не можуть вимкнути `NAV-SAT`.
- На u-blox у прошивці v1.6.19+, якщо приймач відповідає, але до DR1 залишається на `SATS=0`, фільтр автоматично надсилає u-blox cold start і reinit STM32 приблизно через 2 хвилини. Він не запускає `UBX_RESET=3` автоматично. Після автоматичної спроби він логує `If stuck: bench UBX_RESET=3`; використовуйте стендові кроки recovery нижче лише якщо acquisition усе ще не відновлюється.
...
- На u-blox у прошивці v1.6.22+ прямий режим autoconfig (`UBX_BAUD=0`) під час boot очищає/завантажує receiver defaults, повторно сканує baud приймача, потім застосовує і зберігає UBX-профіль фільтра. Ручний режим baud (`UBX_BAUD>0`) пропускає це boot-очищення і залишає профіль приймача під контролем оператора або виробника.
...
Прошивка v1.6.22+ не запускає `UBX_RESET=3` автоматично, але direct u-blox
autoconfig може автоматично очищати збережену конфігурацію u-blox під час boot
перед завантаженням receiver defaults і записом профілю фільтра. Для
...
**Mosaic X5 / passive NMEA**: STM32 не надсилає команд reset до приймача. Якщо no-fix триває, прошивка лише повторює passive NMEA baud scan. Reset або переконфігурацію Mosaic робіть через Septentrio RxTools/Web UI.
