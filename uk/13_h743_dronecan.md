...
Normal no-fix, boot guard, або GNSS reconfiguration теж можуть suppress
`Fix2/Auxiliary`, але ці стани лишають `NodeStatus` health `OK` і показують
output block тільки через vendor status bits та onboard screen. Це прибирає
ArduPilot `PreArm: DroneCAN: Node 42 unhealthy!`, коли GPS просто ще не
готовий.
