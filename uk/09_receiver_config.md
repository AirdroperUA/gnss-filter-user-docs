...
- Авто-конфігурація тепер примусово ставить `COM1` у `460800` перед увімкненням повного
  COM1-профілю та зберігає цей baud через `SAVECONFIG`. Це не дає важкому змішаному
  потоку NMEA/binary працювати на нижчій збереженій швидкості, де вихід може деградувати.
