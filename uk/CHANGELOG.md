...
Firmware-only реліз сумісності та recovery для u-blox. Це публічний follow-up
до v1.6.12; проміжні тестові збірки 1.6.13/1.6.14 включені в цей реліз і не
показуються як окремі версії прошивки на сервері.

### Прошивка

- **u-blox no-fix diagnostics**: коли `SATS=0` або позиційний фікс невалідний,
  прошивка опитує `UBX-MON-RF`, `UBX-SEC-SIG` і `UBX-CFG-GNSS` та логує
  `ubxpvt ...`, `ubxrf ...`, `ubxsig ...` і `ubxgnss ...` у Mission Planner.
  Ці рядки показують NAV-PVT fix flags, antenna/RF state, explicit
  jamming/spoofing state, якщо приймач це підтримує, і enabled GNSS
  constellation configuration.
- **Ручна команда reset для u-blox**: новий віртуальний параметр `UBX_RESET`
  видно у Mission Planner, але він не зберігається у flash. `1` запускає hot
  start, `2` запускає cold start, `3` очищає збережену конфігурацію u-blox у
  BBR/Flash, завантажує defaults, перезапускає приймач і дає STM32 заново
  ініціалізувати GNSS.
- **Без автоматичного factory reset**: руйнівне очищення конфігурації приймача
  виконується тільки через адресований `PARAM_SET` до STM32-фільтра.
- **Зрозуміліші no-fix логи**: поле `fix=...` у періодичних логах тепер показує
  тільки вік валідного позиційного фікса. Якщо приймач живий, але фікса немає,
  `fix=...` буде старим, а `nav=...` свіжим.
- **u-blox SNR recovery тепер враховує сам NAV-SAT stream**: прошивка заново
  надсилає NAV-SAT configuration лише коли NAV-SAT кадри відсутні або
  застарілі. Якщо NAV-SAT надходить, але має нуль придатних C/N0, приймач
  перебуває в acquisition або не має придатного RF, тож прошивка лише логує
  діагностику і не переписує конфігурацію.
- **Boot-time VALSET profile звужено**: UART2 NAV-SAT більше не записується
  постійно під час старту. UART2 залишається у RAM-only recovery path для плат,
  де NAV-SAT справді відсутній на лінку, який слухає фільтр.

---

## v1.6.12 — 2026-05-21

Цей реліз виправляє випадки, коли F9P/u-blox показував здоровий fix і
SATS, але Mission Planner довго логував `SNR=NA`.

### Прошивка

- **Виправлення F9P NAV-SAT / SNR**: UBX-парсер тепер приймає великі
  `NAV-SAT` кадри від приймачів з великою кількістю супутників у полі
  зору, замість відкидання кадрів на межі 512 байтів.
- **Діагностика SNR**: при тривалому `SNR=NA` u-blox тепер виводить
  `snrdbg`, щоб було видно, чи NAV-SAT не надходить, кадр завеликий,
  пошкоджений або приходить без придатних C/N0.
- **Відновлення NAV-SAT**: recovery-команда заново вмикає NAV-SAT через
  legacy `CFG-MSG` і `CFG-VALSET` на UART1/UART2, що покриває F9P carrier
  boards, які використовують другий UART.
- **Свіжість scoring**: SNR, pseudorange residual і SNR-correlation більше
  не використовують застарілі NAV-SAT дані після переходу статусу в
  `SNR=NA`.
