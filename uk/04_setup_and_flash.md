...
- Для параметрів STM32-фільтра: у Mission Planner тільки читання (Read/Refresh). Будь-які зміни робіть лише через `tune_cli.py`.
...
## 5) Обов'язковий крок введення в експлуатацію

Перед звичайною експлуатацією виконайте один раз перевірку наскрізного GPS-шляху до FC:

1. Підключіться до фільтра (`SYSID=42`) і встановіть:
   - `FCGPS_FWD = 1`
2. Дочекайтеся стабільного GNSS у логах фільтра (`GNS nav` зростає, є супутники).
3. Переконайтеся, що FC бачить GNSS на сторінці/статусі GPS.
4. Поверніть робочий режим:
   - `FCGPS_FWD = 0`

`FCGPS_FWD=1` використовуйте лише для діагностики підключення.  
Для штатного антиспуф-режиму тримайте `FCGPS_FWD=0`, щоб у DR1 фільтр блокував пересилання GNSS.
