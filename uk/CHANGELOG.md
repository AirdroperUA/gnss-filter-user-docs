...
Dev/test оновлення таймінгу відновлення SNR для u-blox. v1.6.15 залишається
...
- **Вибір dev-збірки прошиває правильний образ**: сервер і оновлений
  застосунок передають окремі display/raw-version поля, тому оператор бачить
...
- **SNR guard залишається безпечним щодо хибного спрацювання**: `SNR_EN=1`
  досі не може перевести в DR1 через `SNR=NA` або застарілі SNR-дані.
  Застарілий SNR ігнорується прямим SNR guard і оцінкою spoof-confidence.
...
- **Ручна команда відновлення u-blox**: новий віртуальний параметр `UBX_RESET`
  ...
  конфігурацію u-blox у BBR/Flash, завантажує defaults, перезапускає приймач
