...
версією за замовчуванням на сервері; обирайте `v1.6.22 (dev)` лише коли
підтримка просить перевірити цю збірку.

### Прошивка

- **Autoconfig стартує з receiver defaults**: у прямому режимі u-blox з
  `UBX_BAUD=0` boot тепер переводить приймач на цільовий baud, очищає/завантажує
  receiver defaults, повторно сканує baud, потім застосовує і зберігає
  UBX-профіль фільтра.
- **Manual baud/gateway режим виключений**: якщо `UBX_BAUD>0`, фільтр пропускає
  boot clear/autoconfig шлях і використовує налаштований baud напряму. Це
...
Корекція тестової прошивки для відновлення SNR на u-blox. v1.6.15 залишається
стабільною версією за замовчуванням на сервері. Замінена `v1.6.22 (dev)`.
...
Тестова прошивка лише для швидшого відновлення SNR на u-blox. v1.6.15
залишається стабільною версією за замовчуванням на сервері. Замінена
`v1.6.21 (dev)`.
...
- **Startup/no-fix поведінка залишається консервативною**: no-fix, нуль C/N0 і
  ...
  не крутила конфігурацію приймача, поки RF acquisition нестабільний.
...
Тестова прошивка лише для відновлення u-blox у стані без фікса. v1.6.15
залишається стабільною версією за замовчуванням на сервері; обирайте
...
- **Ручний fallback залишається задокументованим**: якщо автоматичне
  відновлення при нулі супутників не повертає acquisition, стендовий fallback
...
Тестова прошивка лише для стабілізації SNR на u-blox. v1.6.15 залишається
стабільною версією за замовчуванням на сервері; обирайте `v1.6.18 (dev)` лише
коли підтримка просить перевірити цю збірку.
...
- **Перевірка статусу прошивки лише за ліцензійним ключем**: desktop app
...
- **Звіти про статус прошивки тепер автентифіковані**: сервер повертає
  ...
  невідповідні tokens. Для автентифікованих лічильників статусу використовуйте desktop app `2026.05.29.9`
  або новіший.
- **Успішні Activate/Update тепер показують зрозумілий popup**: після
  фінальної точки фізичного успіху desktop app відкриває OK-діалог із версією
...
- **Успішний Recover Board тепер показує наступний крок**: після перевірки
  чистого flash і знятого RDP desktop app відкриває OK-діалог із підказкою
...
- **Ліцензії з кількома платами тепер можуть оновлювати вибрану плату**:
  коли на ліцензії зареєстровано кілька плат, desktop app і CLI просять
...
- **Завантаження app з landing page тепер веде на зафіксований published
  EXE**: server більше не залежить від рухомого GitHub `latest` redirect, тому
  public download link не має віддавати старіший cached desktop app після release.
- **Підказки power-cycle більше не радять підключати BlackPill USB-C**:
  desktop app, CLI та self-install docs тепер кажуть зняти й повернути живлення
...
- **Підпис прошивки тепер відділяє stable default від вибраних dev builds**:
  desktop app показує серверне значення як **Типова стабільна прошивка**
  замість "Server firmware version" / "Latest firmware", тому вибір dev
  прошивки у dropdown більше не виглядає як невідповідність зі стабільним
  default, показаним нижче.
- **Dropdown прошивок тепер позначає перший пункт як stable default**:
  desktop app показує `vX.Y.Z (stable default)` замість `vX.Y.Z (latest)`,
  а dev-збірки залишаються окремими пунктами `vX.Y.Z (dev)`.
- **License status на landing page використовує таке саме stable-default
  формулювання**: public license checker тепер підписує серверне значення як
...
- **Dev-прошивка відокремлена від стабільної**: список прошивок залишає
  `v1.6.15 (stable default)` типовим стабільним варіантом і показує
