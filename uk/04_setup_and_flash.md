...
- На платі має бути встановлена звичайна польотна збірка, а не сервісна `*_usbmaint`.
...
Якщо використовується інший тип приймача (наприклад UM980/UM981), протокол GPS на FC має відповідати виходу вашого приймача.
...
- Захист DR1 реалізований блокуванням пересилання GNSS із входу приймача на GPS UART FC.
...
- Для UM980/UM981 налаштуйте `COM1` так, щоб його протокол і baud відповідали потоку, який STM32 буде пересилати на FC.
...
2. Дочекайтесь здорового GNSS у логах (`age` малий, супутники присутні, немає постійного no-fix стану).
...
- Якщо після сервісного оновлення `A11/A12` перестали працювати як FC GPS UART, перевірте, що на платі більше не залишився `*_usbmaint` образ.
