...
- **Виправлено очищення PCROP/SPRMOD під час оновлення захищених плат**:
  ...
  `BOR_LEV=3`. STM32F4 дозволяє очищати SPRMOD лише під час переходу
  RDP1->RDP0, тому додаток більше не намагається очищати SPRMOD після цього і
  не позначає справні плати як hardware-stuck.
- **Повторна спроба працює для плат, уже залишених у RDP0/SPRMOD1**: якщо
  ...
  заблокованою PCROP, додаток спочатку виконує легальний RDP recycle, потім
  очищує `nWRP`, просить power cycle і повторює запис прошивки.
...
- **CLI retry має parity з desktop app**: якщо command-line updater знаходить
  ...
  тепер виконує той самий легальний RDP recycle, очищує `nWRP`, просить power
  cycle і один раз повторює комбінований запис прошивки.
