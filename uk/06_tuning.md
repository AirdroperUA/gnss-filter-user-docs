...
| `GNSS_SWAP` | Зарезервовано для STM32F401 (runtime swap не підтримується, тримайте 0) | 0 | 0 | 0 |
...
- **GNSS_SWAP**: Зарезервовано для STM32F401. Runtime-обмін GNSS RX/TX не підтримується цією апаратною/драйверною комбінацією; тримайте значення 0.
...
- Після будь-якого `set` зачекайте до **30-45 секунд** перед наступними змінами. У цей час фільтр може тимчасово перезапускатися/переналаштовуватися (може виглядати як перезавантаження), коротко зникати зі списку систем у GCS або не відповідати на `get`/`list`.
...
- `GNSS_SWAP` на STM32F401 зарезервований і має залишатися `0`.
