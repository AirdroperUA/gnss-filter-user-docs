...
- `GNSS_TYPE` - виняток для runtime-поведінки: значення зберігається одразу, але починає діяти лише після перезавантаження.
- `UBX_BAUD` також застосовується тільки після перезавантаження.
...
- Перезавантажуйте STM32 (`NRST` або power cycle) для `GNSS_TYPE`, `UBX_BAUD` або якщо поведінка не відповідає новим значенням.
