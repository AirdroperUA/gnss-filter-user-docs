...
Якщо після `set` команда `get` повертає `No reply`, зачекайте до **30-45 секунд** і повторіть `get` (можливий короткий розрив під час збереження/перезапуску/перепідключення).
