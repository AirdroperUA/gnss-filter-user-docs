...
- On u-blox firmware v1.6.20+, if a healthy fix already exists and NAV-SAT later disappears, the first NAV-SAT re-enable runs after about 8 seconds instead of the normal 30-second stale-SNR window. Startup and no-fix cases still use the slower path.
