...
- **DR1**: захисний режим. Живе передавання GNSS на GPS UART FC заблоковане. Якщо `DR_NOFIX=1` і `NMEA_NOFIX=1`, натомість передаються періодичні NMEA NO_FIX маяки.
...
- Зазвичай ви бачите дві сусідні строки:
  ...
  - строка зведення GNSS: `data=... fix=... nav=... SATS=... SNR=...`
...
3. Фільтр виходить із DR1 і відновлює DR0 (або припиняє NO_FIX маяки, якщо вони були ввімкнені).
...
- Кожні **60 с** після холодного перезапуску, якщо фікс досі відсутній: повторна спроба холодного перезапуску.
...
Після `FRESET` STM32 чекає завершення boot приймача, повторно знаходить активну
швидкість GNSS і знову застосовує повний профіль UM980 з повторним `SAVECONFIG`.
