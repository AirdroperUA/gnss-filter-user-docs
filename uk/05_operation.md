...
- На u-blox у прошивці v1.6.18+ штатний захищений режим зчитує байти FC GPS back-channel, але не пропускає їх у приймач. `fcgps rx` може збільшуватися, але ці байти лише рахуються та відкидаються, тому auto-config записи ArduPilot не можуть вимкнути `NAV-SAT`.
