...
- Після `set` зачекайте приблизно **2-3 секунди**, щоб цикл збереження завершився.
- Рекомендована польотна процедура: після сесії тюнінгу перезавантажте STM32-фільтр (`NRST` або power cycle) перед польотом.
...
## Smoke-тест параметрів

Запускайте smoke-тест після оновлення прошивки або змін у підключенні:

```bash
python tools/param_smoke_test.py --port COM12 --baud 115200
```

Опційно: перевірка збереження після ручного reboot під час тесту:

```bash
python tools/param_smoke_test.py --port COM12 --baud 115200 --check-persistence --reboot-wait 60
```

Покриття за замовчуванням:

- `FCGPS_FWD`
- `DR_LOCK_MS`
- `LOG_MS`

Скрипт повертає початкові значення, якщо не вказано `--no-restore`.
