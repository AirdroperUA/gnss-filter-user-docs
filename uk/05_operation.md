...
- H743 DroneCAN використовує default `EKF_TRIPMS=500`: generic горизонтальні
  дані EKF мають залишатися невалідними щонайменше 500 мс, після чого DR1
  спрацьовує на наступному bad report. Явний `GPS_GLITCHING` спрацьовує негайно
  після boot/rejoin inhibition. F401 зберігає випущений generic-loss default
  `EKF_TRIPMS=0`.
