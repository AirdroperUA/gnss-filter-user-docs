...
## 5) Типові проблеми `tune_cli.py` (Windows)

Якщо `tune_cli.py` відкривається і одразу закривається, або є помилки імпорту:

1. Запускайте з явним інтерпретатором:
...
Якщо після `set` з'являється **No reply for PARAM_NAME**:

- Спочатку перевірте, чи `set` вже повернув `PARAM_NAME=<value>` (це означає, що запис успішний).
- Зачекайте до **30-45 секунд** і повторіть `get` (під час збереження фільтр може коротко перезапускатися/переналаштовуватися).
- Повторіть `get` 2-3 рази (під час автозбереження можливий короткий розрив).
- Переконайтеся, що Mission Planner не утримує той самий COM-порт під час роботи CLI.
