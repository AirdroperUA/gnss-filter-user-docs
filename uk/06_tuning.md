...
### Ворота rejoin та таймінги

- **RJ_BASE_M**: Базові бокові ворота для rejoin. Це стартова допустима відстань між синтетичною позицією та реальним GNSS. Збільшуйте для довгих DR1; зменшуйте, щоб не приймати великі зміщення.
- **RJ_SPD_MULT**: Множник швидкості, який додається до воріт. Збільшує допуск на високих швидкостях. Надто великий — допускає великі стрибки, надто малий — заважає rejoin на швидкості.
- **RJ_EXP_RMPS**: Швидкість розширення воріт у DR1 (м/с). Дозволяє воротам збільшуватися з часом. Високі значення пришвидшують rejoin після довгих DR1, але знижують захист.
- **RJ_EXP_MAXM**: Максимальний ліміт розширення. **Не обмежує дальність польоту**, лише обмежує розмір воріт. Якщо замалий — rejoin може не відбутися; якщо завеликий — приймаються дуже великі відхилення.
- **RJ_DOP2M**: Перевід HDOP у метри для воріт. Вищі значення дають більші ворота при поганій геометрії.
- **RJ_HDOP_MUL**: Додатковий множник для HDOP‑терму. Дозволяє швидко змінювати вплив DOP.
- **RJ_MIN_SATS**: Мінімум супутників для rejoin. Вищі значення — якісніший rejoin, нижчі — швидший.
- **RJ_MAX_HD**: Максимальний HDOP для rejoin. Нижче — суворіше.
- **RJ_LOIT_V**: Поріг малої швидкості для loiter‑умови.
- **RJ_LOIT_MS**: Час перебування нижче порогу, після якого застосовується loiter‑ворота.
- **RJ_LOIT_GM**: Loiter‑ворота (перевизначення розміру воріт після таймера).
- **RJ_STAB_MS**: Вікно стабільності перед стартом rejoin/blend. Довше — безпечніше, але повільніше.
- **BLEND_MS**: Тривалість blend DR1→DR0. Довше — м’якший перехід.
- **DR_LOCK_MS**: Мінімальний lockout після входу в DR1. Під час lockout rejoin заборонено.

### Spoof‑guard (стрибки позиції)
...
### Армінг guard (коли вмикаються перевірки)

- **ARM_MIN_S**: Мінімум супутників для армінгу.
- **ARM_MAX_HD**: Максимальний HDOP для армінгу.
- **ARM_STABMS**: Час стабільності для армінгу.
...
- **ALT_BSEP_M**: Допустиме розходження GNSS vs баро.
- **ALT_BSEPMS**: Час утримання розходження.
- **ALT_RJSEP**: Макс. розходження висоти для rejoin.

### Поведінка DR

- **DR_NOFIX**: Показує NO_FIX у DR1/blend, щоб FC не використовував GNSS.
- **PT_ONLY**: Лише pass‑through режим без синтетики/blend.
...
- **RJ_REQEKF**: Вимагає EKF OK перед rejoin.
- **EKF_TRIPMS**: Час поганого EKF для DR1.
- **EKF_OKRJMS**: Час доброго EKF для rejoin (якщо ввімкнено `RJ_REQEKF`).
...
- **GNSS_SWAP**: Обмін RX/TX GNSS UART у runtime.

### SNR‑guard (близький jammer/spoofer)

- **SNR_EN**: Увімкнення SNR‑guard.
