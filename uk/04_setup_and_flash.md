...
- Для параметрів фільтра Mission Planner використовується лише для читання (Read/Refresh). Для запису використовуйте `tune_cli.py`.
- Після зміни будь-якого параметра фільтра перезавантажуйте STM32 (`NRST` або повне вимкнення/увімкнення) перед польотом.
...
2. Дочекайтесь здорового GNSS у логах (`GNS nav` зростає, супутники присутні).
