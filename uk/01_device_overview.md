...
Фільтр знаходиться між GNSS і FC та виконує три основні задачі:

1. **Парсить GNSS UBX** і оцінює якість сигналу та аномалії.
2. **Керує станом DR (DR0/DR1)**, дозволяючи або блокуючи пересилання GNSS на GPS UART FC.
3. **Контролює пересилання GNSS** на GPS UART FC (блокується у DR1).

## 2) Шляхи даних

Є три UART-з’єднання:

- GNSS і STM32: UBX-вхід для перевірки якості.
- FC MAVLink і STM32: статус, команди та тюнінг.
- FC GPS і STM32: «сире» UBX-пересилання на GPS UART FC.
...
- GNSS пересилається на GPS UART FC.
- DR-охоронці відстежують якість GNSS.

### DR1 (захисний режим)

- Пересилання UBX на GPS UART FC вимкнене.
- `B5` піднімається в high на ~3 секунди при вході.
...
### Приклад B: аномалія SNR (близький jammer/spoofer)

- Кількість супутників висока, але розкид SNR аномально вузький.
- Розкид тримається довше `SNR_HOLDMS`.
- При `SNR_EN=1` вхід у DR1.
...
- завершення lockout (`DR_LOCK_MS`),
- опціональних умов EKF (якщо ввімкнені).

Якщо умови виконані, запускається опціональний blend (`BLEND_MS`), після чого відновлюється DR0.
...
- ворота/час реєднання (`RJ_*`, `BLEND_MS`, `DR_LOCK_MS`),
- анти-спуфінг та висотні захисти (`SP_*`, `ALT_*`),
- SNR-захист (`SNR_*`),
...
- Карта GCS може показувати GNSS-стрибки під час спуфінгу. Основне джерело істини — DR-стан і логи моста.
- Консервативний rejoin безпечніший за швидкий у складних умовах GNSS.
