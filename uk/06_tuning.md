...
- **UBX_BAUD**: Керування baud/автоконфігом u-blox. `0` залишає автоконфіг увімкненим (режим за замовчуванням). Значення `>0` вимикає повний шлях автоконфігу і використовує цей ручний baud. Застосовується після перезавантаження. У ручному режимі фільтр все одно робить best-effort запит на `NAV-SAT`, щоб SNR міг працювати, але якщо приймач ігнорує цей запит, `SNR=NA` залишається нормальним.
