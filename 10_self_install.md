...
`2026.06.23.6` or newer has **Board target -> H743 WeAct DroneCAN** for
...
Authenticated flash-status reporting uses a short-lived server token returned with
...
flag is accepted only to print a disabled-transport error, and the legacy
`--uid` selector is rejected by the supported ST-Link path.
