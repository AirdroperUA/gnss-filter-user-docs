...
- **Некоректні option-byte dumps тепер мають безпечний fallback**: якщо
  recovery може прочитати option bytes, але рядок `RDP` відсутній або не
  парситься, це більше не рахується як чиста плата. Інструмент переходить до
  flash-read RDP probe і зупиняється, якщо захист усе ще виглядає активним.
