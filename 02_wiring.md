...
- To release `A11/A12` (e.g. for diagnostics), set `FCGPS_UART=0` in Mission Planner. That disables the FC GPS UART and puts `A11/A12` into input mode. Set `FCGPS_UART=1` to restore normal FC GPS UART operation. `FCGPS_FWD=1` overrides this and forces the FC GPS UART on for bench validation.
