...
- **DR_NOFIX**: Вмикає показ NO_FIX на FC GPS UART під час DR1/blend. Разом із `NMEA_NOFIX=1` фільтр замість живого GNSS-потоку передає періодичні NMEA no-fix маяки.
...
- **NMEA_NOFIX**: Періодично передає NMEA no-fix маяки на FC GPS UART, коли `DR_NOFIX=1` і активний DR1/blend. Корисно для FC GPS-конфігурацій, що очікують саме NMEA.
...
- **UM980_HIGHDYN**: Режим динаміки UM980/UM981. `0` = `MODE ROVER UAV` (стандарт, за замовчуванням). `1` = `MODE ROVER UAV HIGHDYN` (для агресивних платформ із швидкими змінами кутів). Застосовується після reboot — авто-конфіг надсилає відповідну команду `MODE` до приймача.
