...
## GUI інструмент (альтернатива CLI)

Замість командного рядка можна використовувати **AirDroper GNSS Filter.exe** — графічний інструмент прошивки:

- Підтримує активацію (ST-Link) та оновлення (UART)
- Автоматичне виявлення COM портів
- Автоматична обробка плат з RDP1 (зняття захисту, повне стирання, повторна прошивка)
- Підтримка англійської та української мов

Завантажте `AirDroper GNSS Filter.exe` зі сторінки завантажень магазину.
...
флеш-пам'ять плати. Кожна подія містить час, GPS-координати, кількість
супутників, значення SNR та причину виявлення.

### Завантаження журналів

1. Підключіть USB-UART адаптер (таке ж підключення як для оновлення
   прошивки — TX→PA10, RX→PA9, GND→GND)
2. У GUI інструменті натисніть **Завантажити логи (UART)**
3. Введіть ліцензійний ключ
4. Інструмент зчитає всі збережені події, завантажить їх на хмарну панель
...
Відвідайте `https://gnss-filter.online/dashboard` і введіть ліцензійний
ключ для перегляду:

- **Карта подій** — всі події спуфінгу на карті з кольоровими маркерами
  за причиною виявлення
- **Статистика** — загальна кількість подій, кількість плат, розподіл
  причин
- **Список подій** — хронологія з координатами, висотою, кількістю
  супутників та даними SNR
